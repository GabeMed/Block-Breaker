...
export const getAdjustedVector = (
  ...
  if (absoluteValueOfAngle < minAngle) {
    return surfaceNormalDirection.rotate(
      (absoluteValueOfAngle / angle) * minAngle
    );
  } else if (absoluteValueOfAngle > maxAngle) {
    return surfaceNormalDirection.rotate(
      (absoluteValueOfAngle / angle) * maxAngle
    );
  }
...
export const calculateBallReflectionNormal = (
  hittedBlock,
  ballBounds,
  ballRadius
) => {
  ...
  if (
    ballBounds.top >= blockBounds.top - ballRadius &&
    ballBounds.bottom <= blockBounds.bottom + ballRadius
  ) {
...
export const withNewBallDirection = (
  currentBallDirection,
  surfaceNormalDirection,
  getUpdatedGameStateWithBall
) => {
  const distortion = getDistortedDirection(
    currentBallDirection.reflect(surfaceNormalDirection)
  );
